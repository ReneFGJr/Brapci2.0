	...
	if (count($rlt) == 1) {
		...
		switch($proto)
			...
			case 'ufrgs':
				...
				$headers = 'From: Brapci <00282381@ufrgs.br> ' . "\r\n" .
				           'Reply-To: rene.gabriel@ufrgs.br' . "\r\n" .
...
function enviaremail_ufrgs($para, $assunto, $texto, $de, $anexos = array()) {
    echo "OK1";
	if (!is_array($para)) {
		$para = array($para);
	}
	...
	$config = Array('protocol' => 'smtp', 'smtp_host' => 'smtp.ufrgs.br', 'smtp_port' => 465, 'smtp_user' => '00282381', 'smtp_pass' => 'Axpz@693', 'mailtype' => 'html', 'charset' => 'iso-8859-1', 'wordwrap' => False);
	$CI -> load -> library('email', $config);
